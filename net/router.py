...
def make_handler(ctx: dict):
    ...
    def _pace_watchlist(direction):
        ...
        for (track, car_ord), seqs in combos.items():
            ...
            magnitude = delta if direction == "worse" else -delta
            if magnitude < thresh:
                continue
        ...
        # Biggest mover first; cap so Home doesn't bloat.
        candidates.sort(key=lambda c: c["delta_s"], reverse=True)
        candidates = candidates[:3]

        # Enrich: car name from FORZA_CARS + the track's overall PB lap
        # (any car) so the row outline matches the track shape.
        if candidates:
            ...
            if path.startswith("/static/"):
                ...
                with db_lock:
                    ...
                    try:
                        ...
                        # Track record (any car) at this circuit — lets the hero
                        # grade an all-cars best (the bigger celebration) above a
                        # mere car PB. See docs/specs/home-actionable-and-celebrate.md.
                        if last_sess and last_sess.get("track"):
                            tr = conn.execute(
                                "SELECT MIN(best_lap_time_s) AS pb "
                                "FROM sessions "
                ...
                payload = {
                    ...
                    "pb_at_track_any_car_s": pb_track_any,
                ...
                # (track, car) combos where the recent 3 sessions are SLOWER
                # than the prior 3 — the "what should I work on?" answer.
                # See docs/specs/home-regression-watchlist.md (brainstorm).
                writer.write(_http_response("200 OK", "application/json",
                                            json.dumps(_pace_watchlist("worse")).encode()))

            elif path == "/home/improvement-watchlist":
                # Mirror of the regression watchlist: (track, car) combos
                # where the recent 3 sessions are FASTER than the prior 3 —
                # the "what's working" wins. See
